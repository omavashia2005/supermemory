...
if (!datasetPath) throw new Error("Usage: --dataset FILE [--output DIR] [--modes comma,list] [--judge-module FILE] [--limit N] [--max-typesafe-requests N]");
...
await runLongMemEval({ datasetPath, outputDirectory: value("--output") ?? "longmemeval-results", modes, judge, limit: value("--limit") ? Number(value("--limit")) : undefined, topK: topK ? Number(topK) : undefined, maxTypeSafeRequests: value("--max-typesafe-requests") ? Number(value("--max-typesafe-requests")) : undefined });
