...
export interface Limits { initialCandidates:number; maxCandidates:number; maxHops:number; maxGraphReads:number; maxTypeSafeRequests:number; maxInputTokens:number; contextTokens:number; timeoutMs:number; retries:number }
...
export interface RetrievalResult { memories:Memory[]; context:string; trace:Trace }
